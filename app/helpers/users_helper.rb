module UsersHelper
  ...
  def social_sharing_url(user, provider)
    case provider
      ...
      when 'facebook'
        "https://www.facebook.com/dialog/feed?app_id=419105881446208&link=#{user.public_url}&redirect_uri=#{user.public_url}&description=#{t('users.show.public_explanation_html', fullname: user.fullname)}&display=popup"
      when 'twitter'
